...
```bash
lambda-69$ sam local invoke TheGameFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-69$ sam local start-api
lambda-69$ curl http://localhost:3000/
```
...
```bash
lambda-69$ sam logs -n TheGameFunction --stack-name lambda-69 --tail
```
